...
public class nublogger_mamba extends PApplet {
...
/** 
 * Nublogger Mamba
 * This is the latest version of the nublogger software as of 1.10.09
 ...
 * 
 * The difference between this code and the Cascabel version is that there's more of a user interface.  The user can
 * see the latest values received by each sensor, and also select a sensor and view an autoscaled graph of the last
 * hundred samples.
 */
...
int leftAlign=15;
int topAlign=20;
int textSpacing=20; //the spacing between rows of text

int xoffset=200;
int yoffset=200;

int graphWindowX=200;
int graphWindowY=200;
...
//class used to store sample data
class Sample{
  float value;
  float scaledValue;  //the value scaled for display on the screen
  Sample(float a)
   {
       value=a;
       scaledValue=yoffset-yscale*a;
   }
...
class Sensor{
  ...
  int index;
  boolean selected;
  final float scalingFactor=1;   //vertical scaling factor for the graphed data
  LinkedList samples;
  ...
  Sensor(String newName, String value, String sensorUnits)
  {
    name=newName;
    configured=false;
    selected=false;
...
//  File configFile, dataFile;
  ...
  if(skipAutoDetection)    //just go ahead and use known defaults if I'm debugging.
  {
    serialPortNumber=0;
  }
  else    
...
//  while(true)   //I don't trust draw to run properly--I'm putting in my own forever loop
 // {
  ...
  text("sensors we're listening to",leftAlign,topAlign);
  for(int count=0;count<allSensors.size();count++)
  {
    Sensor a=(Sensor) allSensors.get(count);
    if(a.selected==true)  //if this sensor is selected, graph its last 100 samples
    {
      fill(255,0,0);
      graphData(a);   
    }
      ...
      fill(255,255,255);
    text(a.name,leftAlign,topAlign+((count+1)*textSpacing));
    if(a.samples.size()>0) 
    { 
      Sample mySample=(Sample)a.samples.get(a.samples.size()-1);
      text(mySample.value, leftAlign+textWidth(a.name),topAlign+((count+1)*textSpacing));
      text(a.units, leftAlign+textWidth(a.name)+textWidth(Float.toString(mySample.value))*2,topAlign+((count+1)*textSpacing));
    }
    fill(255,255,255);
  }
...
public void autoscale(Sensor mySensor)
{
  float maximum=-100000, minimum=100000;
  Sample mySample;
  for(int i=0;i<mySensor.samples.size();i++)
  {
    mySample=(Sample)mySensor.samples.get(i);
    if(mySample.value>maximum)
      maximum=mySample.value;
    if(mySample.value<minimum)
      minimum=mySample.value;
  }
  float range=maximum-minimum;
  if(range==0)  //fix a divide-by-0 error  
    range=1;
  range*=1.5f;  //it looks crappy if we fill the entire graph window
  yscale=graphWindowX/range;
  for(int i=0;i<mySensor.samples.size();i++)
  {
    mySample=(Sample)mySensor.samples.get(i);
    mySample.scaledValue=yoffset-yscale*(mySample.value-minimum);
    mySensor.samples.set(i,mySample);
  }
  
}

public void graphData(Sensor mySensor)
{
    autoscale(mySensor);
    stroke(0,0,100);
    strokeWeight(2);
    ...
    if(mySensor.samples.size()>1)
    {
      for(int i=1;i<mySensor.samples.size();i++)
      {
        oldSample=(Sample)mySensor.samples.get(i-1);
        newSample=(Sample)mySensor.samples.get(i);
        line((i-1)*xscale+xoffset, oldSample.scaledValue, i*xscale+xoffset, newSample.scaledValue);
      }
    }
    smooth();
}     
...
public void addData(String[] splitMessage)
{
  ...
  {
    allSensors.add(new Sensor(splitMessage[NAME],splitMessage[VALUE],splitMessage[UNITS]));   //add the name to the sensors list
...
/*  String timestamp=day()+"/"+month()+"/"+year()+","+hour()+":"+minute()+":"+second()+",";
  ...
  for(int i=0;i<allSensors.size();i++)   
  {
    ...
    {
      float value=Float.parseFloat(splitMessage[VALUE]);   
      mySensor.samples.add(new Sample(value));
      if(mySensor.samples.size()>numSamples)
        mySensor.samples.removeFirst();
      allSensors.set(i,mySensor);
...
sensors.
*/
public void writeToConfiguredSensors(String[] splitMessage)
{
  try{
...
public void checkMouseOver()
{
  for(int count=0;count<allSensors.size();count++)
  {
    ...
    Sample b;
    if(a.samples.size()>0)
      b=(Sample)a.samples.get(a.samples.size()-1);
    else
      b=new Sample(0);
    int width=(int)textWidth(a.name+a.units)+2*(int)textWidth(Float.toString(b.value));
    if((mouseX>leftAlign)&&(mouseX<(leftAlign+width))&&(mouseY>(topAlign+((count*textSpacing))))&&(mouseY<(topAlign+((count+1)*textSpacing))))
    {
      a.selected=true;
      allSensors.set(count,a);  //if the user clicked on this sensor, set it to 'selected'
    }
    else
      if(a.selected==true)   //if the user didn't clicked on a different sensor name, and this one was previously 
                             //selected, de-select it
      {
        a.selected=false;
        allSensors.set(count,a);
      }
  }
}
...
{
    checkMouseOver();  //look to see if the mouse was clicked over a particular sensor name, and select that sensor
                      //if it was
}


  static public void main(String args[]) {
    PApplet.main(new String[] { "nublogger_mamba" });
  }
}
