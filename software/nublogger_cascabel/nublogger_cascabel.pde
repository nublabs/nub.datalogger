...
String allSensorsOutputFile="/Users/nikki/Documents/Processing/nublogger_processing/output-all.csv";
String configuredSensorsOutputFile="/Users/nikki/Documents/Processing/nublogger_processing/output.csv"
...
void addData(String[] splitMessage)
{
  ...
  for(i=0;i<sensors.size();i++)    //go through the sensors list and see if we have this sensor's name stored
  {
    mySensor=(sensor)sensors.get(i);
  ...
  for(i=0;i<configuredSensors.size();i++)    //go through the sensors list and see if we have this sensor's name stored
  {
    ...
    if(splitMessage[NAME].equals(mySensor.name))
      inCofiguredSensors=true;
  }
  
  if((!inSensors)&&(!inConfiguredSensors))   //this is a completely new sensor
  {
    sensors.add(new Sensor(splitMessage[NAME]));   //add the name to the sensors list
  ...
  {
    writeToConfiguredSensors(splitMessage[NAME]);
    writeToAllSensors(splitMessage[NAME]);
  }
...
void writeToAllSensors(String[] splitMessage)
{
  ...
  //write the timestamp
  output.print(day()+"/"+month()+"/"+year()+",");   //print out the date
  output.print(hour()+":"+minute()+":"+second()+",");  //print out the time in 24-hour format
  ...
  */
  for(int i=0;i<allSensors.size();i++)   
  {
    ...
    if(mySensor.name.equals(splitMessage[NAME]))  //this is the sensor from splitMessage
      output.print(splitMessage[VALUE]+","+splitMessage[UNITS]+",");   //print out the data
    else
      output.print(",,");    //just print out blank spaces as placeholders
  }
  output.println();
  output.close();
}
